classdef NLCont2D < handle
    ...
    methods
        ...
        function Kt = K(obj, du)
            ...
            ed = du(obj.edof(:, 2:end));
            for elm = 1:obj.nelm
                % Computes the defgrad, if updated lagrangian use old ef
                ef = obj.defgrad(obj.matrices{elm, 2}, ed(elm, :)');
                ...
                % Using the above data the element stiffness matrix can be
                % computed
                Kelm = obj.stiffness(obj.matrices{elm, 2}, obj.matrices{elm, 3}, obj.matrices{elm, 1}, obj.t, D, ef, es);
                
                % Inserting the element stiffness matrix into the correct pos
                k0 = ((elm - 1)*nne + 1); ke = (elm*nne);
                X(k0:ke, 1) = Kelm(:);
            end
            Kt = sparse(obj.I, obj.J, X);
        end
